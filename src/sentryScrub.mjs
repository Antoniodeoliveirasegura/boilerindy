...
// Google API keys: AIza + 35 url-safe chars, not JWT/Bearer/hex-shaped. Kept
// after the move to Groq so an old key in a stale breadcrumb is still redacted.
const GOOGLE_KEY_RE = /AIza[0-9A-Za-z_-]{35}/g
// Groq API keys (GROQ_API_KEY): gsk_ followed by ~52 alphanumerics. Not hex- or
// JWT-shaped, so none of the patterns above would catch one on its own.
