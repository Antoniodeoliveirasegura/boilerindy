...
- **Node.js 22 (22.22.2 or newer) or Node.js 24 (24.15 or newer)** - check with `node -v`. The project runs on Node 22: both `.nvmrc` files (repo root and `boilerindy-react/`) hold just `22`, so CI and Render follow the newest 22.x release. Install it from [nodejs.org](https://nodejs.org) or run `nvm install` in either folder, which downloads and selects the newest 22.x. `nvm use` never downloads one: it switches to the newest 22.x already on your machine even when that is older than 22.22.2, and CI cannot catch this because it always runs the newest 22.x. So run `nvm install` rather than `nvm use` in the repo, again after each Node 22 release, and confirm `node -v` prints 22.22.2 or newer. Both `package.json` files declare the supported range, `^22.22.2 || ^24.15.0`, in `engines` (jsdom 30 sets that floor). pnpm 11 itself fails on Node older than 22.13. On a Node that runs pnpm but is outside the range, pnpm 11.6 prints an `Unsupported engine` warning on `pnpm install` and `pnpm run` and carries on; pnpm's docs say a project `engines` mismatch should fail the install, so do not rely on that warning.
