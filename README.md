...
36. *(optional)* `db/supabase-user-id-indexes.sql` - adds a `user_id` index to `board_posts`, `marketplace_listings`, `guide_recommendations` and `study_group_members`, the four tables that cascade from `users` without one (issue #212); needs steps 1, 13, 15 and 16. Index-only, idempotent and safe to rerun: it reads, writes and deletes no rows. Speeds up `GET /api/marketplace/mine`, `GET /api/me/study-groups`, and the per-table `DELETE ... WHERE user_id = $1` that Postgres runs on each of the four when a student deletes their account. Until it runs nothing breaks: those two reads and the account-deletion cascade scan the table.
