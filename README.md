...
- **Node.js 22 (22.22.2 or newer) or Node.js 24 (24.15 or newer)** - check with `node -v`. Install from [nodejs.org](https://nodejs.org) or use `nvm`: `nvm use` in the repo root or in `boilerindy-react/` picks 22.22.3, the version both `.nvmrc` files pin and CI runs. Both `package.json` files declare the supported range, `^22.22.2 || ^24.15.0`, in `engines` (jsdom 30 sets that floor); pnpm warns but still installs on a Node outside it.
