...
- **Node.js 22 (22.22.2 or newer) or Node.js 24 (24.15 or newer)** - check with `node -v`. Install from [nodejs.org](https://nodejs.org) or use `nvm`: `nvm install` in the repo root or in `boilerindy-react/` installs and selects 22.23.2, the version both `.nvmrc` files pin and CI runs (`nvm use` selects it again in a new shell). Both `package.json` files declare the supported range, `^22.22.2 || ^24.15.0`, in `engines` (jsdom 30 sets that floor). pnpm 11 itself needs Node 22.13 or newer and exits with an error on an older Node. On a Node that runs pnpm but is outside the range, pnpm 11.6 prints an `Unsupported engine` warning and still installs; pnpm's docs say a project `engines` mismatch should fail the install, so do not rely on that warning.
